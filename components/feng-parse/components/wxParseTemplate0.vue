<template>
	...
	<block v-if="node.node == 'element'">
		...
		<!--a类型-->
		<view v-else-if="node.tag == 'a'" @click="wxParseATap(node.attr,$event)" :class="node.classStr" :data-href="node.attr.href" :style="node.styleStr" style="display: inline; border-bottom: 1px solid #6E7AD5;">
	...
	export default {
		...
		methods: {
			wxParseATap(attr,e) {
				const {
					href
				} = attr;// TODO currentTarget才有dataset
				if (!href) return;
