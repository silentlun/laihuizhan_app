			...
			loadDetail(){
				uni.request({
					...
					success: (res) => {
						...
						jiaotong = jiaotong.replace(/\<img/gi, '<img style="max-width:100%;height:auto" ')
						this.jiaotong = htmlParser(jiaotong);
						let sheshi = data.sheshi.replace(/\<p/gi, '<p class="ep"');
						sheshi = sheshi.replace(/\<img/gi, '<img style="max-width:100%;height:auto" ')
